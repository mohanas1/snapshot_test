...
PC_IP="${1:-${PC_IP:-}}"
PC_USER="${2:-${PC_USER:-nutanix}}"
PC_PASSWORD="${3:-${PC_PASSWORD:-}}"
...
# Post-completion checks (downstream workflow signals)
...
# Execute SSH command (same pattern as log-fetch script)
ssh_exec() {
    local host=$1
    local user=$2
    local password=$3
...
setup_kubeconfig() {
    ...
    if [ -z "$PC_IP" ]; then
        log_error "PC_IP is not set. Cannot fetch kubeconfig from PC."
        log_error "Set PC_IP (and optionally PC_USER/PC_PASSWORD) before running."
        exit 1
    fi

    if [ -n "$PC_PASSWORD" ] && ! command -v sshpass &>/dev/null; then
        log_warn "PC_PASSWORD provided but sshpass not installed; interactive SSH prompt may appear."
    fi

    # Fetch kubeconfig from remote PC using mspctl (same logic as log-fetch script)
    log_info "Fetching kubeconfig from PC ${PC_USER}@${PC_IP}..."
    if ! ssh_exec "$PC_IP" "$PC_USER" "$PC_PASSWORD" \
        "/usr/local/nutanix/cluster/bin/mspctl cls kubeconfig nc" > "$KUBECONFIG_FILE" 2>/dev/null; then
        log_error "Failed to fetch kubeconfig from PC ${PC_USER}@${PC_IP}"
        exit 1
    fi
...
# Get pod name by prefix in a namespace (prefer running)
get_pod_by_prefix() {
    local namespace="$1"
    local prefix="$2"
    local pod_name=""

    pod_name=$(kubectl --kubeconfig="$KUBECONFIG_FILE" get pods -n "$namespace" --field-selector=status.phase=Running -o jsonpath='{.items[*].metadata.name}' 2>/dev/null \
        | tr ' ' '\n' | grep "^$prefix" | head -1)

    if [ -z "$pod_name" ]; then
        pod_name=$(kubectl --kubeconfig="$KUBECONFIG_FILE" get pods -n "$namespace" -o jsonpath='{.items[*].metadata.name}' 2>/dev/null \
            | tr ' ' '\n' | grep "^$prefix" | head -1)
    fi
...
# Monitor downstream workflows after data-loader completion.
monitor_post_completion_workflows() {
    ...
    log_info "Watching downstream pods for cost-update signals for $WAIT_AFTER_COMPLETION seconds"
    log_info "Script waits full window before next iteration"
    ...
    while [ $elapsed -lt $WAIT_AFTER_COMPLETION ]; do
        ...
        if [ $found_count -eq $total_patterns ] && [ $all_patterns_seen -eq 0 ]; then
            all_patterns_seen=1
            log_success "All downstream post-completion workflow signals observed."
    ...
    if [ $found_count -eq $total_patterns ]; then
        log_success "Post-completion wait ended with all signals observed ($found_count/$total_patterns)."
        return 0
    else
        log_warn "Post-completion window ended before all signals were seen."
...
echo "  - PC IP: ${PC_IP:-<not-set>}"
echo "  - PC User: $PC_USER"
